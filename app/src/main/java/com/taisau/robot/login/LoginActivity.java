...
import com.taisau.robot.contact.ContactHttpClient;
...
public class LoginActivity extends UI implements OnKeyListener {
    ...
    private ClearableEditTextWithIcon loginAccountEdit;//账号
    private ClearableEditTextWithIcon loginPasswordEdit;//密码
    ...
    private boolean registerMode = false; // true:注册模式；false：登錄模式
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        onParseIntent();//踢出，下線通知
        initRightTopBtn();//初始化登錄
        setupLoginPanel();//登錄麵板
        setupRegisterPanel();//註冊面板
    }
...
//                    login();
    ...
    private void setupRegisterPanel() {
        ...
        switchModeBtn.setVisibility(View.GONE);
    ...
    private void myLogon() {
        ...
        final String account = loginAccountEdit.getEditableText().toString();
        final String password = tokenFromPassword(loginPasswordEdit.getEditableText().toString());

        OkHttpUtils.post()
                .url(Constant.LOGIN_URL)
                .addParams("loginName", account)//ts
                .addParams("loginPwd", password)//123456
//                .addParams("eqCode", Constant.EQCODE)
                .build()
                .execute(new StringCallback() {
                    @Override
                    public void onError(Call call, Exception e, int i) {
                        LogUtils.e("网络请求有异常:" + e);
                    }

                    @Override
                    public void onResponse(String s, int i) {
//                        LogUtils.e("返回的数据信息：" + s);
                        ...
                        try {
                            ...
                            if (code == 0) {
                                ...
                                if (jsonObjectRts != null) {
                                    String login = jsonObjectRts.optString("login");
                                    String loginName = jsonObjectRts.optString("loginName");
                                    String token = jsonObjectRts.optString("token");
                                    if (Boolean.valueOf(login)) {
                                        //登录网易云平台
                                        login(loginName, token);
                                    }
                                }
                            } else {
...
//        final String token = tokenFromPassword(loginPasswordEdit.getEditableText().toString());//ead34ec7eaec0522f69d43ca1f898265
        ...
        loginRequest = NimUIKit.login(new LoginInfo(account, token), new RequestCallback<LoginInfo>() {
            ...
            public void onSuccess(LoginInfo param) {
                ...
                saveLoginInfo(account, token);//保存賬號和token
