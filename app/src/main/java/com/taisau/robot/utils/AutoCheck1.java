 ...
 * <p>
 * 示例使用代码：
 * AutoCheck1.getInstance(getApplicationContext()).check(initConfig, new Handler() {
 *
 * @Override public void handleMessage(Message msg) {
 * if (msg.what == 100) {
 * AutoCheck1 autoCheck = (AutoCheck1) msg.obj;
 ...
 * });
 */
public class AutoCheck1 {

    private static AutoCheck1 instance;
     ...
     * @return
     */
    public static AutoCheck1 getInstance(Context context) {
        if (instance == null || AutoCheck1.context != context) {
            instance = new AutoCheck1(context);
        }
        return instance;
    }
    ...
    public void check(final InitConfig initConfig, final Handler handler) {
        Thread t = new Thread(new Runnable() {
            @Override
            public void run() {
                AutoCheck1 obj = innerCheck(initConfig);
                isFinished = true;
                synchronized (obj) { // 偶发，同步线程信息
                    Message msg = handler.obtainMessage(5, obj);
    ...
    private AutoCheck1 innerCheck(InitConfig config) {
    ...
    private AutoCheck1(Context context) {
