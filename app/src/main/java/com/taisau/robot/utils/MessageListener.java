...
public class MessageListener implements SpeechSynthesizerListener, MainHandlerConstant {
    ...
    private static OnWakeupStartListener onWakeupStartListener;

    public static void setOnWakeupStartListener(OnWakeupStartListener onWakeupStartListener) {
        MessageListener.onWakeupStartListener = onWakeupStartListener;
    }

    private static OnWakeupStopListener onWakeupStopListener;

    public static void setOnWakeupStopListener(OnWakeupStopListener onWakeupStopListener) {
        MessageListener.onWakeupStopListener = onWakeupStopListener;
    }
...
//        onWakeupStartListener.wakeupStart();
//        LogUtils.e("唤醒开始》》》");
    }

    private void test(){
        if (SPUtils.getInstance().getBoolean("meijunrui",true)){
            UnityPlayer.UnitySendMessage("Main Camera", "Baidu_speechsynthesis", "true");//关闭语音识别
        }
        SPUtils.getInstance().put("meijunrui",true);
    }
...
//        onWakeupStopListener.wakeupStop();
//        LogUtils.e("唤醒结束》》》");
        sendMessage("播放结束回调, 序列号:" + utteranceId);
        UnityPlayer.UnitySendMessage("Main Camera", "Baidu_speechsynthesis", "false");//开启语音识别
    }
